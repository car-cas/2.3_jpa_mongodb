...
* **Where is the findAll method implemented?**
    ...
    El método findAll se implementa en la clase CrudRepository y MongoRepository. 
...
* **How many products are returned by the findByDescriptionContaining query? Why?**
    ...
    2 Products, porque retorna la paginación de 2 products.

* **Which are the collection names where the objects are stored? Where are those names assigned?**

    Costumers y Product, se asignan en @Document(collection = "name").
 
 * **Is it possible to implement all of them?**
